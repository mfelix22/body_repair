...
class PurchaseRequestController extends Controller
{
    ...
    public function close(Request $request, PurchaseRequest $purchaseRequest)
    {
        ...
        if (!$user || (!$isRequester && !$isAdmin)) {
            return redirect()->route('purchase_requests.show', $purchaseRequest)
                ->with('error', 'Only the PPB/PPJ maker or an administrator can close this request.');
        }

        if (!in_array($purchaseRequest->status, ['completed', 'printed'])) {
            return redirect()->route('purchase_requests.show', $purchaseRequest)
                ->with('error', 'Only PPB/PPJ that have been received by purchasing can be closed.');
        }
        ...
        $purchaseRequest->update([
            'status' => 'closed',
            'cancellation_reason' => $validated['close_reason'] ?? null,
        ]);
